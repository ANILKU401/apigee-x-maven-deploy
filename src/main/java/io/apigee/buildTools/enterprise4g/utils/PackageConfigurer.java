...
public class PackageConfigurer {
	...
	public static void configureSharedFlowPackage(String env, File configFile)
        ...
        XMLFileListUtil listFileUtil = new XMLFileListUtil();
        List<File> fileList = listFileUtil.getSharedFlowsFiles(configFile);
        ...
        for (int i = 0; i < fileList.size(); i++) {
            ...
            try {
                Policy configTokens = conf.getConfigbyEnv(env)
                        .getSharedFlowFileNameMatch(fileList.get(i).getName());
                // logger.info("\n\n=============Replacing config tokens for Environment {}, for policy or sharedflow file name {}================\n",env,fileList.get(i).getName());
                if (configTokens != null) {
                    logger.info(
                            "=============Replacing config tokens for Environment {}, for sharedflow file name {}================\n",
                            env, fileList.get(i).getName());
                    xmlDoc = replaceTokens(xmlDoc, configTokens);
                    DOMSource source = new DOMSource(xmlDoc);
                    StreamResult result = new StreamResult(fileList.get(i));
                ...
                logger.error(
                        "\n\n=============No config tokens found for Environment {}, for sharedflow file name {}================\n",
                        env, fileList.get(i).getName());
                throw e;
            }
        ...
        // get the list of files in policies folder
        fileList = listFileUtil.getPolicyFiles(configFile, "sharedflowbundle");
        for (int i = 0; i < fileList.size(); i++) {
        ...
        // get the list of files in sharedflowbundle folder
        fileList = listFileUtil.getSharedFlowBundleFiles(configFile);
    ...
    public static void configureSharedFlowPackage1(String env, File configFile)
            throws Exception {
